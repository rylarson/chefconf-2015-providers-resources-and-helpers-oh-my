...
class Chef::Provider::ExampleHlwrp < Chef::Provider::LWRPBase  #Class name must match what is in self.resource_name
  ...
  def load_current_resource
    ...
    @current_resource = Chef::Resource::ExampleHlwrp.new(@new_resource.name)
    #Normally, kickstart-hlwrp is the name of my cookbook and chef will convert the name to a class so it becomes KickstartHlwrp.  This is because there is a '-' or a '_'.
    #If I were to create something other than default, say service.rb in my provider/resource.  This would then be KickstartHlwrpService.new and you
    #would access it in your recipes with kickstart_lwrp_service.

    #But because we created a custom name, we use ExampleHlwrp with the same rules being applied for a '-' and a '_'.
  ...
  def create
    ...
    t = template "/var/#{ new_resource.name }" do  #This is an instance of another resource, template.  You have access to methods, instance variables, etc., in another resource.
      ...
      source 'tmpfile.erb'
      cookbook 'kickstart-hlwrp'   #if you are providing the template file, you must state that it is coming from your cookbook.  If another cookbook calls this resource
  ...
  def delete
    ...
    t = template "/var/#{ new_resource.name }" do
      source 'tmpfile.erb'
      cookbook 'kickstart-hlwrp'   #If you are providing the template file, you must state that it is coming from your cookbook.
      action :delete
    end
